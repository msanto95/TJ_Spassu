<?php
    ...
    $stmt = $pdo->prepare('SELECT * FROM "livro" WHERE codl ='.$cod);
    $stmt->execute();
    $resData=$stmt->fetchAll();

    foreach($resData as $rowData){
        $anopublicacao = $rowData['anopublicacao'];
    ...
    <script>
        $(document).ready(function(){
            $('#gravar').click(function() {
                ...
                $.post('gravaLivro.php', {orig: 1, codigo: codigo, editora: editora, edicao: edicao, anopub: anopub}, function(data) {
            ...
            <div class="border border-dark w-400 mt-2 bg-light">
                <div class="p-3">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control w-75 border-dark" id="titulo" disabled value="<?php echo $titulo; ?>">
                    </div>
                    <div class="form-group">
                        <label for="editora">Editora</label>
                        <input type="text" class="form-control  w-75 border-dark" id="editora" value="<?php echo $editora; ?>">
                    </div>
                    <div class="form-group">
                        <label for="edicao">Edição</label>
                        <input type="text" class="form-control  w-25 border-dark" maxlength="2" onkeypress="somenteNumeros(event)" id="edicao" value="<?php echo $edicao; ?>">
                    </div>
                    <div class="form-group">
                        <label for="anopub">Ano de publicação</label>
                        <input type="text" class="form-control w-25 border-dark" maxlength="4" onkeypress="somenteNumeros(event)" id="anopub" value="<?php echo $anopublicacao; ?>">
                    </div>
                </div>  
